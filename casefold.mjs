...
lines
  ...
  .forEach((line) => {
    ...
    switch (status) {
      ...
      case 'F': {
        full.set(parseInt(code, 16), mapping.split(' ').map((value) => String.fromCharCode(parseInt(value, 16))).join(''));
        break;
      }
      case 'S': {
        simple.set(parseInt(code, 16), mapping.split(' ').map((value) => String.fromCharCode(parseInt(value, 16))).join(''));
        break;
      }
      case 'T': {
        special.set(parseInt(code, 16), mapping.split(' ').map((value) => String.fromCharCode(parseInt(value, 16))).join(''));
        break;
      }
      default: {
        throw Error('Unexpected status value.');
      }
    }
  });
 ...
 * @param {string} value
 */
export const simple_casefold = (value) => {
  assert(typeof value === 'string');
  let casefolded = '';
  for (let i = 0, l = value.length; i < l; i += 1) {
    const code = value.charCodeAt(i);
    if (common.has(code) === true) {
      casefolded += common.get(code);
      continue;
    }
    if (simple.has(code) === true) {
      casefolded += simple.get(code);
      continue;
    }
    casefolded += value.charAt(i);
  }
  return casefolded;
};
 ...
 * @param {string} value
 */
export const full_casefold = (value) => {
  assert(typeof value === 'string');
  let casefolded = '';
  for (let i = 0, l = value.length; i < l; i += 1) {
    const code = value.charCodeAt(i);
    ...
    if (full.has(code) === true) {
      casefolded += full.get(code);
      continue;
    }
    casefolded += value.charAt(i);
  }
 ...
 * @param {string} value
 */
export const special_casefold = (value) => {
  assert(typeof value === 'string');
  let casefolded = '';
  for (let i = 0, l = value.length; i < l; i += 1) {
    const code = value.charCodeAt(i);
    ...
    if (special.has(code) === true) {
      casefolded += special.get(code);
      continue;
    }
    if (full.has(code) === true) {
      casefolded += full.get(code);
